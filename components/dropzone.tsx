...
export const Dropzone = () => {
  ...
  const onDrop = async (acceptedFiles: File | undefined) => {
    if (!acceptedFiles) {
      setToastMessage("Format requis : xlsx ou xls");
      setToastType("error");
      return;
    }
    ...
    if (
      ...
      setToastMessage("Le fichier doit être au format .xlsx ou .xls.");
      setToastType("error");
      return;
    }
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop: (acceptedFiles) => {
      if (acceptedFiles.length > 1) {
        setToastMessage("Veuillez déposer un seul fichier.");
        setToastType("error");
        return;
      }
        ...
        >
          ...
          <div className="dz-message m-0! flex flex-col items-center">
            ...
            <span className="mb-5 block w-full max-w-[320px] text-center text-sm text-gray-700 dark:text-gray-400">
              Cliquer ou déposer le fichier des appels ici (Excel).
            </span>
